...
// ----------- < req 5 demo > -----------
int MetricExecutive::Req5(std::vector<std::string> files1)
{
	...
	std::cout << "		" << "- actions for each rule that support building the Abstract Syntax Tree (AST) that represents a single file's scope structure and identifies the type and line number extend of each scope.\n" << std::endl;
	...
	try	{		
		...
		std::cout << std::endl << "Function \t\t\t" << "-------" << "Line Count" << "----" << "Complexity";
		std::cout << "\n ***********************************************************";
...
int main(int argc, char* argv[])
{
	...
	int check = MetricExecutive::Req5(files);	
	if (check == 2) std::cout << "File not present";
